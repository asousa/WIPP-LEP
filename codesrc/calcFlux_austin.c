    ...
    for(ti=0; ti<NUM_TIMES; ti++) {
      ...
      if(mag != 0) {
         ...
         //               thrown by asin() function
            ...
            // --------- Austin's careful re
            float arg1 = (P/PI)*sin(2*x);
            float arg_in1 = (x - alpha_eq)/mag;
            float arg_in2 = 0;
            if (arg_in1 < 0) { arg_in2 = -1.0*asin(abs(arg_in1)); }
            else { arg_in2 = asin(arg_in1); }
          
            float arg2 = arg_in2 + (float)(PI/2);
            ...
            float arg2 = arg_in2 + (float)(PI/2);
            float arg_in3 = float(mag*mag-pow((x-alpha_eq),2))

            g = arg1*(arg_in1*arg2 + sqrt(arg_in3));
